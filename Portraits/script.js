...
/* itemsSelector */
{
  ...
  /* при использовании расскоментировать стили в (.often-questions .accordion-block) и закомментировать (.often-questions .accordion-block.active-content)*/
...
const calc = (size, material, options, promocode, result) => {
  ...
  sizeBlock.addEventListener('change', culcFunction);
  materialBlock.addEventListener('change', culcFunction);
  optionsBlock.addEventListener('change', culcFunction);
  promocodeBlock.addEventListener('change', culcFunction);
};
...
const modals = () => {
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      ...
      if (!display) {
        /* если ни одно модальное окно не открыто, то покажем необходимое окно */
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', () => {
      ...
      if (!brnPressed && window.pageYOffset + document.documentElement.clientHeight >= scrollHeight) {
...
const scrolling = upSelector => {
  ...
  /* Скролл с применение requestAnimationFrame */

  let links = document.querySelectorAll('[href^="#"]'),

  /* ищем все ссылки, которые начинаются с #, то есть локальные ссылки */
  speed = 0.2;
  links.forEach(link => {
  ...
  /* узнаем какое расстояние было пролистано вниз пользователем */

  /* if (this.hash !== '') {
      event.preventDefault();
      let hashElement = document.querySelector(this.hash), */

  /* получение элемента к которому будет скролиться страница */

  /*  hashElementTop = 0;   */

  /* сколько еще нужно пролистать пикселей до родителя этого хэш элемента */

  /* while (hashElement.offsetParent) {
      hashElementTop += hashElement.offsetTop; */

  /* offsetTop  - позволяет определить сколько пикселей остается до верхней грнаицы родительского элемента от хэш элемента */

  /* hashElement = hashElement.offsetParent;
  }       */

  /* цикл перебирает всех родителей искомого элемента и позволяет узнать сколько пикселей необходимо будет отлистать */
  ...
  /* такое условие гарантирует, что мы точно долистали до момента, который нам был нужен */
